...
specs <-  specr::setup(
  data= life_expec_dat,
  y = c("life_expectancy"),
  x = c("gdp", "log(gdp)", "I(gdp^2)"),
  model = c("lm", "glm"),
  controls = c("population", "alcohol", "bmi"),
  subsets = as.list(distinct(life_expec_dat,status))
)
...
multiv_model_out_broom <- purrr::map_dfr (multiv_model_out[29:31],
                                          broom::tidy,
                                          .id= "model_type")
...
intercepts <- multiv_model_out_broom %>%
...
mod_table <- modelsummary::modelsummary(multiv_model_out[29:31], 
                                        output = "kableExtra",
                                        fmt = fmt_significant(1),  # 2 significant digits instead of fixed decimal places
                                        estimate  = "{estimate}",
                                        statistic = "conf.int",
                                        coef_omit = "Intercept",
                                        coef_rename=c("gdp"="Gdp", 
